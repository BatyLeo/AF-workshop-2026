...
# ╔═╡ eb40cc66-8a69-4abc-bab6-044ca32cf449
using PlutoUI

# ╔═╡ 33771f88-3d46-4e9f-ad92-a30d4739e521
begin
    using Random
...
# ╔═╡ c72ac4ad-9e95-4957-a807-705f3c0a14f3
using DecisionFocusedLearningBenchmarks

# ╔═╡ b529c6ce-7ab4-4b21-92a1-733e57ae732e
using Flux: Flux, Dense, Chain, relu, Adam

# ╔═╡ 7a704253-5382-44f1-96ad-ed6240904068
using JuMP, HiGHS

# ╔═╡ d1aad732-b671-48ce-a4a2-1e2b36892e8e
using InferOpt

# ╔═╡ feb4615c-75ee-4886-91a3-fc955561edae
using DecisionFocusedLearningAlgorithms

# ╔═╡ 1a89287c-b5b2-4651-839a-c2f114468898
md"""
# Decision-Focused Learning demo: contextual stochastic knapsack

A guided, from-scratch implementation of a **benchmark** and an **end-to-end training pipeline** using the JuliaDecisionFocusedLearning ecosystem.

In this demo, we will:
1. Define a contextual stochastic knapsack problem using the `DecisionFocusedLearningBenchmarks` interface.
2. Train a DFL policy to solve this problem, using `DecisionFocusedLearningAlgorithms.`
3. Evaluate the resulting policy.
"""

# ╔═╡ ba65863d-683c-4e9a-af07-823c2f9e172d
TableOfContents()

# ╔═╡ 19d213ab-4c84-4e7f-93c6-380b2ba6932a
md"""
## The problem

We have ``n`` items with known **weights** ``c`` and a knapsack **capacity** ``C``.
Each item ``i`` has an unknown **utility** ``\xi_i``, we only observe context features correlated with these values.

- **Instance**: integer item weights ``c`` and capacity ``C``. Base utility values ``u_{base} \sim U[0,1]^n``.
- Observed **context**: ``x_{raw} \sim \mathcal{N}(0, I_d)``. The model sees ``x = [u_{base}, \ c/c_{\max},\ x_{raw}]``.
- **Scenario**: realized values ``\xi = u_{base} + M x_{raw} + \text{noise}`` (``M`` is unknown to the learner).

**Goal:** find a knapsack policy ``\pi`` that gives good utility in practice.
"""

# ╔═╡ e1d1a9c0-0001-4abc-8def-000000000001
md"""
## The DFL policy ``\pi_w``

```math
\xrightarrow[\,x\,]{\text{context}}\;
\boxed{\,\varphi_w\,}\;
\xrightarrow[\,\theta\in\mathbb{R}^{n}\,]{\text{values}}\;
\boxed{\,f:\ \operatorname*{arg\,max}_{c^\top y\,\le\,C}\ \theta^\top y\,}\;
\xrightarrow[\,y\in\{0,1\}^{n}\,]{\text{decision}}
```

- A **statistical model** ``\varphi_w`` maps context ``x`` to predicted values ``\theta``; the
- **combinatorial oracle** ``f`` turns them into a knapsack decision.
- **Learning problem**: Find ``w`` so that *decisions* ``y=\pi_w(x)=f(\varphi_w(x))`` are good for any input instance.
"""

# ╔═╡ 5201148a-090b-48b5-b313-2731a23fb224
md"""
## 1. Define the benchmark type

A contextual stochastic benchmark is an `AbstractStochasticBenchmark{true}` (exogenous: the
uncertainty does not depend on our decisions).
"""

# ╔═╡ 52a2d839-8ef9-4ae2-8de0-75556d68eb3b
const DFLB = DecisionFocusedLearningBenchmarks

# ╔═╡ 19d4ffbf-58ec-41f5-8928-d86f477f12ad
struct ContextualStochasticKnapsackBenchmark <: AbstractStochasticBenchmark{true}
    n::Int
    d::Int
    M::Matrix{Float32}
    noise_std::Float32
    ...
    function ContextualStochasticKnapsackBenchmark(;
        seed=0,
        n::Int=10,
        d::Int=5,
        noise_std::Float32=0.1f0,
        c_max::Int=5,
        weight_frac::Float64=0.5,
    )
        rng = Random.Xoshiro(seed)
        M = randn(rng, Float32, n, d)
        return new(n, d, M, noise_std, c_max, weight_frac)
    end
end

# ╔═╡ 73b82ebf-7e18-48c8-8a62-2db38a4c9068
bench = ContextualStochasticKnapsackBenchmark(; n=10, d=5)

# ╔═╡ 5522d76a-719b-4fff-9248-8656a9ec0281
is_exogenous(bench)

# ╔═╡ 1b70ca4a-4d33-49e1-b8a9-9826cb50c64f
md"""
## 2. The benchmark interface: generating data

These are the *basic* methods of the `DecisionFocusedLearningBenchmarks` interface: how to
sample an instance, its context and a scenario, and how to score a decision. No optimizer is
needed yet. This is exactly the recipe you will follow yourself in the practice session.
"""

# ╔═╡ 702a117e-f6c6-4f1e-8c04-3577744dffec
DFLB.is_minimization_problem(::ContextualStochasticKnapsackBenchmark) = false

# ╔═╡ 1e4cebfd-3cbb-4317-bbd7-63175cd173e6
struct KnapsackInstance
    C::Int
    c::Vector{Int}
    u_base::Vector{Float32}
end

# ╔═╡ 32f54943-9025-413e-baff-037bfb2e6b7f
function DFLB.generate_instance(
    bench::ContextualStochasticKnapsackBenchmark, rng::Random.AbstractRNG
)
    (; c_max, weight_frac, n) = bench
    c = rand(rng, 1:c_max, n)
    C = max(c_max, round(Int, weight_frac * sum(c)))
    u_base = rand(rng, Float32, n)
    instance = KnapsackInstance(C, c, u_base)
    return DataSample(; instance)
end

# ╔═╡ cbbb0253-1345-4da8-af51-7b28d14ac4ab
instance_sample = generate_instance(bench, Xoshiro(67))

# ╔═╡ d73d7901-639e-4783-955c-d9edd3b2ce58
instance_sample.context.instance

# ╔═╡ 1e848482-d46c-4340-b92c-4f151e258d08
instance_sample.instance

# ╔═╡ 81f069af-7c33-4821-92ca-0879a9e95305
instance_sample.extra

# ╔═╡ 62460e08-b26e-4002-9ad4-7317f24551c0
instance_sample.instance

# ╔═╡ d620a6b7-4d14-4de1-ad23-072396d3cf11
function DFLB.generate_context(
    bench::ContextualStochasticKnapsackBenchmark,
    rng::Random.AbstractRNG,
    instance_sample::DataSample,
)
    (; c, u_base) = instance_sample.instance
    c_feat = Float32.(c ./ bench.c_max)
    x_raw = randn(rng, Float32, bench.d)
    x = vcat(u_base, c_feat, x_raw)
    return DataSample(instance_sample; x=x, x_raw)
end

# ╔═╡ 91c66b21-41cb-40f5-be66-200eb3c16541
sample = generate_context(bench, Xoshiro(67), instance_sample)

# ╔═╡ 483662d5-61c4-418d-8fef-72058a647873
sample.x

# ╔═╡ fa084118-7a4c-410a-8a13-a42e77cb9162
sample.context

# ╔═╡ 68c0dc55-3e71-4c1c-99ad-9625b0256b21
function DFLB.generate_scenario(
    bench::ContextualStochasticKnapsackBenchmark,
    rng::Random.AbstractRNG;
    instance::KnapsackInstance,
    x_raw::Vector{Float32},
)
    θ_true = instance.u_base + bench.M * x_raw
    θ_true += bench.noise_std * randn(rng, Float32, bench.n)
    return θ_true
end

# ╔═╡ 59afd7fe-13e3-4cf4-831d-3b2eea6f2dd9
generate_scenario(bench, Random.default_rng(); sample.context...)

# ╔═╡ 0ce7a585-2010-46b0-a4ad-143b50977416
function DFLB.objective_value(
    ::ContextualStochasticKnapsackBenchmark, sample::DataSample, y, scenario
)
    return dot(scenario, y)
end

# ╔═╡ 2407880b-ffdc-427f-a584-6706273d6453
generate_dataset(bench, 10; nb_scenarios=2, contexts_per_instance=2)

# ╔═╡ f0000000-0000-4000-8000-000000000003
md"""
## 3. The statistical model

The statistical model ``\varphi_w`` maps a context ``x`` to predicted values
``\hat\theta \in \mathbb{R}^n``. We instantiate it later, when
building the policy.
"""

# ╔═╡ 2ae7ebc1-3b05-4ffe-812c-d57d7168f5b2
function DFLB.generate_statistical_model(
    bench::ContextualStochasticKnapsackBenchmark; seed=nothing
)
    isnothing(seed) || Random.seed!(seed)
    in_dim = 2 * bench.n + bench.d   # [u_base; c_feat; x_raw]
    return Chain(Dense(in_dim => 20), relu, Dense(20 => bench.n))
end

# ╔═╡ bcc86210-4130-4041-a96d-1a2a222cf741
initial_model = generate_statistical_model(bench; seed=67)

# ╔═╡ e19b30bc-538f-49fa-887c-3075847ecdea
θ = initial_model(sample.x)

# ╔═╡ b62875f7-b498-4906-aa2c-6218b03fb803
md"""
## 4. The optimization oracle

The combinatorial oracle can be **any Julia callable** that maps a value vector ``\theta`` to a decision. Here we use a small dynamic program. We then register it on the benchmark via `generate_maximizer`.
"""

# ╔═╡ 7ea1c285-af40-4e21-b108-80a3c9391b4d
function knapsack_dp(θ::AbstractVector; instance, kwargs...)
    n = length(θ)
    c = instance.c
    C = instance.C
    val = fill(-Inf, C + 1)  # best value for each used capacity 0 to C
    val[1] = 0.0
    take = [falses(n) for _ in 0:C]  # best assignment for each used capacity
    for i in 1:n
        cᵢ = c[i]
        θᵢ = θ[i]
        for cap in C:-1:cᵢ
            # add item i if it improves the value at this capacity level
            obj_with_i = val[cap - cᵢ + 1] + θᵢ
            if obj_with_i != -Inf && obj_with_i > val[cap + 1]
                val[cap + 1] = obj_with_i
                take[cap + 1] .= take[cap - cᵢ + 1]
                take[cap + 1][i] = true
            end
        end
    end
...
# ╔═╡ 26fd5cf2-5054-4042-8704-7cf354029d22
knapsack_dp(θ; sample.context...)

# ╔═╡ 29d145e0-5f19-4a00-80f3-589597bcffe4
md"An (slower) alternative could be to use a MIP solver impelmented with JuMP:"

# ╔═╡ c4f1a9b2-0d3e-4c5a-8b7f-1a2b3c4d5e6f
function knapsack_mip(θ::AbstractVector; instance, kwargs...)
    (; c, C) = instance
    n = length(θ)
    model = Model(HiGHS.Optimizer)
    set_silent(model)
    @variable(model, y[1:n], Bin)
    @constraint(model, sum(c[i] * y[i] for i in 1:n) <= C)
    @objective(model, Max, sum(θ[i] * y[i] for i in 1:n))
    optimize!(model)
    return Float32.(round.(value.(y)))
end

# ╔═╡ c367170b-f37f-464e-b34f-38b044fe307c
knapsack_mip(θ; sample.context...)

# ╔═╡ f0000000-0000-4000-8000-000000000004
function DFLB.generate_maximizer(::ContextualStochasticKnapsackBenchmark; dp=true)
    return dp ? knapsack_dp : knapsack_mip
end

# ╔═╡ f8e08c6d-7cce-4ddf-aa9b-07920bcffcb8
maximizer = generate_maximizer(bench; dp=true)

# ╔═╡ f9ba0f5b-e009-4015-b527-18e999fa79da
maximizer(θ; sample.context...)

# ╔═╡ 207c42da-7b83-462b-81e5-46799e7d440b
md"""
## 5. Generate a dataset

`generate_dataset` calls our `generate_instance` → `generate_context` → `generate_scenario`
methods. We pass a `target_policy` that labels each sample with the **anticipative** solution
(the knapsack solved on the realized scenario): this is what the model learns to imitate.
"""

# ╔═╡ f8427435-61a9-49f7-a80d-bf90ad1343c0
function anticipative_policy(sample::DataSample, scenarios::AbstractVector)
    return [
        DataSample(sample; y=knapsack_dp(ξ; sample.context...), extra=(; scenario=ξ)) for
        ξ in scenarios
    ]
end

# ╔═╡ 511a43ff-7bf8-4931-a1fc-a5d9ed5a7d7a
train_data = DFLB.generate_dataset(bench, 100; target_policy=anticipative_policy, seed=1)

# ╔═╡ d09d505a-b6c1-4d71-91c9-98ffbebe1905
train_data[1].extra

# ╔═╡ 16cf0274-1cf0-4212-8005-4c80446c7554
test_data = DFLB.generate_dataset(
    bench, 100; target_policy=anticipative_policy, nb_scenarios=10, seed=101
)

# ╔═╡ 9e7aabe4-09b3-4c86-b128-bb0ee80515e7
md"""
## 6. Train with the Fenchel-Young loss

The knapsack oracle is piecewise constant in ``\theta``, so it has zero gradients almost everywhere. `PerturbedFenchelYoungLossImitation` fixes this by **perturbing**
the predicted values with random noise and averaging. This defines the *perturbed maximizer*:
```math
\begin{aligned}
\hat f(\theta) &= \mathbb{E}_{Z}\!\left[\,\operatorname*{arg\,max}_{y \in \mathcal{Y}(x)}\ (\theta + \varepsilon Z)^\top y\,\right],
\qquad Z \sim \mathcal{N}(0, I),\ \varepsilon\in\mathbb{R}_+ \\
&\approx \frac{1}{M}\sum_{m=1}^{M} \operatorname*{arg\,max}_{y \in \mathcal{Y}(x)}\ (\theta + \varepsilon Z^{(m)})^\top y,
\qquad Z^{(m)} \overset{\text{iid}}{\sim} \mathcal{N}(0, I).\\
\hat f(\theta) &\approx \frac{1}{M}\sum_{m=1}^{M} f(\theta + \varepsilon Z^{(m)}).
\end{aligned}
```

For a target decision ``\bar y`` to imitate, the **Fenchel-Young loss** compares the predicted ``\theta`` against ``\bar y``:
```math
\begin{aligned}
\mathcal{L}(\theta, \bar y) &= \mathbb{E}_{Z}\!\left[\,\max_{y \in \mathcal{Y}(x)}\ (\theta + \varepsilon Z)^\top y\,\right] - \theta^\top \bar y\;\ge\; 0 \\
&\approx \frac{1}{M}\sum_{m=1}^{M} \max_{y \in \mathcal{Y}(x)}\ (\theta + \varepsilon Z^{(m)})^\top y - \theta^\top \bar y.
\end{aligned}
```

Its gradient is exactly the decision error between the perturbed maximizer and
the target:
```math
\nabla_{\theta}\,\mathcal{L}(\theta, \bar y) = \hat f(\theta) - \bar y.
```
"""

# ╔═╡ da62917c-d1e1-42fa-a16b-3e595e2b5002
byhand = let
    model = deepcopy(initial_model)
    perturbed = PerturbedAdditive(maximizer; nb_samples=10, ε=1.0, threaded=true, seed=3)
    ...
    for epoch in 1:50
        epoch_loss = 0.0
        for s in train_data
            l, grads = Flux.withgradient(m -> fyl(m(s.x), s.y; s.context...), model)
            Flux.update!(opt_state, model, grads[1])
            epoch_loss += l
        end
        push!(losses, epoch_loss / length(train_data))
    end
    (; model=model, losses)
end

# ╔═╡ 80e63dbd-7214-4edc-91f4-b9ed7c367606
plot(byhand.losses; marker=:circle)

# ╔═╡ 347cf9c6-284e-4ee4-96ab-03a3894ea66a
md"A `DFLPolicy` bundles the statistical model with the maximizer:"

# ╔═╡ 4bb57ffa-6f86-48ef-af40-d4ea60e1c206
policy = DFLPolicy(deepcopy(initial_model), maximizer)

# ╔═╡ 7be6ed8e-704c-4178-bbb2-8781f6de8e47
algo = PerturbedFenchelYoungLossImitation(; nb_samples=10, ε=1.0, threaded=true, seed=3)

# ╔═╡ 0c1b571a-bc44-4cd6-bee5-67777ecb02da
gap_metric = FunctionMetric(:val_gap, test_data) do ctx, data
    res = 0
    for sample in data
        target_y = sample.y
        θ = ctx.policy.statistical_model(sample.x)
        y = ctx.policy.maximizer(θ; sample.context...)
        scenario = sample.extra.scenario
        target_obj = DFLB.objective_value(bench, sample, target_y, scenario)
        obj = DFLB.objective_value(bench, sample, y, scenario)
        res += 100 * (target_obj - obj) / abs(target_obj)
    end
    return res / length(data)
end

# ╔═╡ 260d4037-9697-4871-a414-748c4c9c764b
history = train_policy!(algo, policy, train_data; epochs=50, metrics=(gap_metric,))

# ╔═╡ 07df5077-e035-4c36-b8d2-808f78ec9c90
md"## 7. Results: the decision gap drops"

# ╔═╡ 6dd32bd5-078d-4c27-8f8b-3b46886512ae
let (epochs_logged, losses) = get(history, :training_loss)
    plot(
        epochs_logged,
        losses;
        xlabel="epoch",
...
# ╔═╡ 5c26dece-1f54-4dbc-8305-5cda102a7db3
let (epochs_logged, gaps) = get(history, :val_gap)
    plot(
        epochs_logged,
        gaps;
        xlabel="epoch",
        ...
        lw=2,
        legend=:topright,
        title="Fenchel-Young imitation on knapsack",
    )
end

# ╔═╡ 570fceb2-22c6-4a53-8eaa-64af641b5ff3
get(history, :val_gap)[2][1]

# ╔═╡ e3787737-2ffb-455f-8452-b6f344925060
minimum(get(history, :val_gap)[2])
...
manifest_format = "2.0"
project_hash = "e8686352e9ea17aaf2234eb201c9cd417e66a628"
...
[[deps.DecisionFocusedLearningAlgorithms]]
deps = ["DecisionFocusedLearningBenchmarks", "DocStringExtensions", "Flux", "InferOpt", "MLUtils", "ProgressMeter", "Random", "Statistics", "UnicodePlots", "ValueHistories"]
git-tree-sha1 = "52f3299d17441a95169af33546fdaddc19826665"
uuid = "46d52364-bc3b-4fac-a992-eb1d3ef2de15"
version = "0.2.3"

[[deps.DecisionFocusedLearningBenchmarks]]
deps = ["Combinatorics", "ConstrainedShortestPaths", "DataDeps", "Distributions", "DocStringExtensions", "FileIO", "Flux", "Graphs", "HiGHS", "Images", "InferOpt", "Ipopt", "IterTools", "JSON", "JuMP", "LaTeXStrings", "LinearAlgebra", "Metalhead", "NPZ", "Printf", "Random", "Requires", "SCIP", "SimpleWeightedGraphs", "SparseArrays", "Statistics", "StatsBase"]
git-tree-sha1 = "54de433c383692645268045e3f3701a9067eb2d6"
uuid = "2fbe496a-299b-4c81-bab5-c44dfc55cf20"
version = "0.6.1"
weakdeps = ["Plots"]

    [deps.DecisionFocusedLearningBenchmarks.extensions]
...
[[deps.InferOpt]]
deps = ["ChainRulesCore", "DensityInterface", "DifferentiableExpectations", "Distributions", "DocStringExtensions", "LinearAlgebra", "Random", "Statistics", "StatsBase", "StatsFuns"]
git-tree-sha1 = "d985c4e4dc0e37010e9f2ae60a203d8c7a9538db"
...
[[deps.MLUtils]]
deps = ["ChainRulesCore", "Compat", "DataAPI", "DelimitedFiles", "MLCore", "NNlib", "Random", "ShowCases", "SimpleTraits", "Statistics", "StatsBase", "Tables"]
git-tree-sha1 = "28477911479c417b9191bfb16cafb42de11cc29c"
...
git-tree-sha1 = "cfc4a317a683a8cd368bde015791055ddd09b9bd"
uuid = "82193955-e24f-5292-bf16-6f2c5261a85f"
version = "0.12.8"

[[deps.SCIP_PaPILO_jll]]
deps = ["Artifacts", "Bzip2_jll", "CompilerSupportLibraries_jll", "GMP_jll", "Ipopt_jll", "JLLWrappers", "Libdl", "MPFR_jll", "OpenBLAS32_jll", "Readline_jll", "Zlib_jll", "bliss_jll", "boost_jll", "oneTBB_jll"]
git-tree-sha1 = "a1d4904f97d594ba96cac60fe2c4f6e5993e295b"
uuid = "fc9abe76-a5e6-5fed-b0b7-a12f309cf031"
version = "1000.0.2+0"

[[deps.SCIP_jll]]
deps = ["Artifacts", "Bzip2_jll", "CompilerSupportLibraries_jll", "GMP_jll", "Ipopt_jll", "JLLWrappers", "Libdl", "MPFR_jll", "Readline_jll", "Zlib_jll", "boost_jll"]
git-tree-sha1 = "67c9ff5915ea849d07491f992067b16a22c7726a"
...
git-tree-sha1 = "45686abfc95dad81f08e41ba062b6649107f43e9"
